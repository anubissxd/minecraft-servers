...
**Nasıl uygulanır:** manifest.json değiştiğinde sıra şudur — (1) manifest'i commit+push et, (2) `git rev-parse HEAD` ile SHA'yı al, (3) `index.json`'daki ilgili `manifest_url`/`patchnotes_url`'ü o SHA ile güncelle, (4) index.json'u commit+push et, (5) sadece `index.json` için jsDelivr purge çağır, (6) **Discord duyurusu** — her paket yayınının son adımı: `ssh root@31.58.91.7 python3 /root/scripts/notify_pack.py "<Paket Adı>" <sürüm> < distribution/<paket>/patchnotes.md` (yama notlarını @everyone ile kanala basar; webhook bilgisi VDS'te, repoya girmez). `index.json`'un kendisi jsDelivr'dan servis edilir ve orada purge güvenilir çalışıyor. `banner_url` nadiren değiştiği için `main` üzerinde kalabilir.
